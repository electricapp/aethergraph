...
is sorted first by what it needs from the machine it runs on, because that —
not the code — is what determines the schedule.
...
roles — fetch, transform, compute — communicating through a ring in VRAM, so
the gather for batch _n+1_ overlaps the aggregation of batch _n_ without stream
...
Removes the sampler from the host critical path entirely: seeds in, node IDs
and offsets out, no round trip.

### K5.3 PTX seqlock reader

The feature table's head/tail seqlock read from device code with correct
acquire/release semantics (`ld.acquire.sys`, `fence.acq_rel.sys`). Lets a
kernel snapshot a live-updating feature table without host mediation, matching
the guarantee the host-side reader already provides.
...
are materialized into a tile; they cannot express a sparse gather. Scope them
to the aggregation kernel and nowhere else.
...
into the GPU's address space via `cudaHostRegister(..., cudaHostRegisterIoMemory)`,
SQ/CQ rings live in VRAM, and doorbells are rung with `st.relaxed.mmio.sys`.
...
plus flow steering rules that deliver matching packets straight into VRAM.
DOCA GPUNetIO is the vendor-supported equivalent.
...
This is the enabling substrate for K1.1 and the natural first Tier B item: it
is where the topology constraints surface, and it fails fast and legibly on a
box that cannot support the rest.
...
degree-weighted heuristic in the userfaultfd pager with measured access
recency. Also runs on any rooted VM.
...
signal. The strategy is therefore to **shrink the surface that needs hardware
to be observed**, until only a few hundred bytes of it remain.
...
Building K1.1 and K2.1 as an untestable doorbell ring around a fully
unit-tested command builder is the difference between a week and a month on
each.
...
1. **K5.3** first — smallest, self-contained, and it establishes the
   device-side memory-model discipline the rest of the compute plane assumes.
2. **K5.2, K5.5, K5.1** — Tier A in dependency order; each has a CPU oracle.
3. **K4.1, K4.2** — cheap, rooted-VM, independent of everything else. Good
   parallel track.
4. **K3.1** — the first Tier B item and the topology canary. If a candidate box
   cannot run this, it cannot run K1.1 or K2.2 either.
5. **K1.1** on top of K3.1; **K1.2/K1.3** alongside it once the namespace is in
   hand.
6. **K2.2**, then **K2.1** — DEVX QP construction is the apprenticeship for
   GPU-side WQE construction.
7. **K5.4**, **K2.3**, **K3.3**, **K3.2** — hardware-gated, schedule them when
   the part is available.

The single highest-leverage action is securing one bare-metal box with a
ConnectX and a spare NVMe namespace, and verifying its ACS/IOMMU topology
before any Tier B code is written. That fact determines whether the crown-jewel
items are days-hard or months-hard.
...
| Speculative sampling with rollback | Under counter-based RNG the sample for a given seed set is deterministic, so next-batch work is *pre-execution*, not speculation. Build the prefetch; the misprediction and rollback machinery has nothing to do. |
