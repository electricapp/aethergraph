...
Hot f16 decode paths dispatch on F16C at runtime in either wheel; the v3 wheel
additionally vectorizes every other loop at compile time. Building from source
with `RUSTFLAGS="-C target-cpu=native" maturin develop --release` is the
ceiling.

## Huge pages

The mmap paths request `MADV_HUGEPAGE` themselves; make sure transparent huge
pages are at least `madvise`:
