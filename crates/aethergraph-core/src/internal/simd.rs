...
impl F16Decoder {
    /// Perform the CPU feature queries once.
    #[inline]
    pub fn resolve() -> Self {
        Self {
            #[cfg(target_arch = "x86_64")]
    ...
    pub fn convert(self, src: &[u8], dst: &mut [f32]) {
        ...
        {
            if self.avx512f {
                // SAFETY: `resolve` verified avx512f is present, and that
                // answer cannot change for the life of the process; slice
                // lengths were asserted at entry.
