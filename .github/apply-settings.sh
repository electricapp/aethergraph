#!/usr/bin/env bash
# Reconcile GitHub-side settings for electricapp/aethergraph to match
...
#
# NOTE: `environments` is empty today — aethergraph publishes no artifacts
# yet. Add a `pypi-publish` environment entry here (wait timer + reviewer +
# `v*.*.*` tag policy) when a release/publish pipeline lands.

set -euo pipefail

REPO="electricapp/aethergraph"
